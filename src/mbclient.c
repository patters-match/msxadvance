...
//	u16 slaves;
	u16 ie;
	u32 emusize1,emusize2,romsize,biossize;
...
//	emusize=((u32)(&Image$$RO$$Limit)&0x3ffff)+((u32)(&Image$$RW$$Limit)&0x7fff);
	...
	if(pogoshell) romsize=pogosize+sizeof(romheader);
	else romsize=sizeof(romheader)+*(u32*)(findrom(romnum)+4);
	biossize=0x8000 + sizeof(romheader);
	if(emusize1+biossize+romsize>max_multiboot_size) return 3;
...
//ex=purple->white
	...
	xfer(emusize1+emusize2+romsize+biossize);		//transmission size..
	xfer((emusize1+emusize2+romsize+biossize)>>16);
	...
	for(;emusize2;emusize2-=2)		//send second part of emu
		xfer(*(p++));
	p=(u16*)g_BIOSBASE-sizeof(romheader)/2;
	for(;biossize;biossize-=2)		//send BIOS
		xfer(*(p++));

	if(pogoshell)
	{
		mb_header.identifier=EMUID;
		mb_header.filesize=pogosize;
	...
	for(;romsize;romsize-=2)		//send ROM
		xfer(*(p++));
	i=0;
transferEnd:
	REG_IE=ie;
