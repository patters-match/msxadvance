...
### Enhancement
MSXAdvance is a solid emulator which runs very well on GBA. Many of the best shoot 'em ups in gaming got a start on this system (Nemesis, Nemesis II, Nemesis III, Salamander, Zanac A.I., Twin Bee, Parodius, R-Type). The SCC sound chip was also very advanced for an 8 bit micro, so these titles remain very playable today. However, MSXAdvance is let down by one significant usability issue - you have to manually select the ROM mapper for each game, and you need to remember which ones they use, which means a lot of fiddling about in menus. Not anymore though!
