...
To minimise the changes in the emulator's ARM ASM I repurposed the upper 3 bits of the ```emuflags``` 4 byte word passed by **main.c** to **cart.s**, meaning that the ```spritefollow``` half-word within is reduced from 16 bits to 13 bits wide. AFAIK sprite follow should still work ok (max value is now 8191).
